...
def detectar_drift_imc(df: pd.DataFrame) -> dict:
    """Detecta cambios en la distribución del IMC (test KS)."""
    imc_actual = df["IMC"].dropna()
    mean_actual = imc_actual.mean()
    mean_base   = BASELINE["IMC_mean"]
    cambio_pct  = abs(mean_actual - mean_base) / mean_base

    alerta = cambio_pct > DRIFT_IMC_THRESHOLD
    ...
    return {
        ...
        "umbral_pct"   : DRIFT_IMC_THRESHOLD * 100,
        "alerta"       : alerta,
...
def detectar_drift_clases(df: pd.DataFrame, col_pred: str = TARGET) -> dict:
    ...
    prop_clase2_actual = vc.get(2, 0)
    prop_clase2_base   = BASELINE["proporcion_clase2"]
    cambio             = abs(prop_clase2_actual - prop_clase2_base)

    alerta = cambio > DRIFT_CLASS2_THRESHOLD
    ...
    return {
        ...
        "umbral"              : DRIFT_CLASS2_THRESHOLD,
        "alerta"              : alerta,
        "distribucion_actual" : {
            str(k): round(float(v), 4) for k, v in vc.items()
        },
        "mensaje": (
            f"ALERTA: Clase 2 cambió {cambio:.3f} puntos" if alerta else "OK"
        ),
    }
...
def generar_reporte(df: pd.DataFrame, ruta_salida: str = None) -> dict:
    ...
    reporte = {
        ...
        "estadisticas" : {
            "IMC_mean"  : round(df["IMC"].mean(), 4),
            "IMC_std"   : round(df["IMC"].std(), 4),
            "Peso_mean" : round(df["Peso"].mean(), 4),
            "Talla_mean": round(df["Talla"].mean(), 4),
        },
    }

    # Resumen de alertas
    alertas = [v["alerta"] for k, v in reporte.items()
               if isinstance(v, dict) and "alerta" in v]
    reporte["hay_alertas"] = any(alertas)
    reporte["n_alertas"]   = sum(alertas)
...
def actualizar_baseline(df: pd.DataFrame) -> None:
    ...
    BASELINE = {
        ...
        "IMC_std"          : round(df["IMC"].std(), 4),
        "Peso_mean"        : round(df["Peso"].mean(), 4),
        "proporcion_clase0": round(vc.get(0, 0), 4),
        "proporcion_clase1": round(vc.get(1, 0), 4),
        "proporcion_clase2": round(vc.get(2, 0), 4),
    }
    # Guardar baseline actualizado
